...
public class ProductService  {
	...
	private void initDatabase2ES() {
		...
		if(page.getContent().isEmpty()) {//如果ES之后没有信息的话
			List<Product> products= productDAO.findAll();//查询数据库中所有数据
			for (Product product : products) {
				productESDAO.save(product);//保存到es中
			}
		}
	}

	/**
	 *	改用elasticsearch 全文检索方法实现商品的搜索
	 * */
